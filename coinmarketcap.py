...
for code in top_twenty_codes:
    if code in kraken_coins:
        kraken_invest.append(code)
    elif code in poloniex_coins:
        poloniex_invest.append(code)
    elif code in bittrex_coins:
        bittrex_invest.append(code)
    else:
        print(top_twenty[code] + ' is not in any exchange')


total_coin_number = len(kraken_invest) + len(poloniex_invest) + len(bittrex_invest) 
kraken_percent = len(kraken_invest)/total_coin_number 
poloniex_percent = len(poloniex_invest)/total_coin_number 
bittrex_percent = len(bittrex_invest)/total_coin_number 
# len(kraken_invest) + len(poloniex_invest) + len(bittrex_invest) + len(extras) = 20 hopefully
